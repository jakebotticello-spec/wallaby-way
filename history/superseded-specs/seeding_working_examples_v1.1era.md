...
*purpose: the canonical calibration cases every reading window boots with. A reader calibrates the bar far faster against concrete cases than against the abstract rule — and **every window calibrating to the SAME examples is what keeps the bar consistent across divided, blind labor.** Three cases: a clean FENCE, a TEXTURE that is also a fence (the overlap case), and a NO-CARD case (the most frequent call a reader makes, and the discipline most likely to erode under reading fatigue).*
*authority: subordinate to The Progenitor — these examples ILLUSTRATE the §3 bar, they do not extend it. If an example and the doctrine ever disagree, the doctrine wins and the example is wrong. Grounded verbatim in The Progenitor §1, §2.2, §3.*

---

## How to read these

Each example shows: the **situation in the floor** (what the reader is looking at), the **call** (card or no-card, which kind), the **why** (the bar applied out loud), and the **proposal shape** (what gets handed to synthesis, if anything). The point is not to memorize three cases — it's to internalize the *test* they each exercise, so the thousands of judgment calls that aren't these three land consistently.
...
```
*Laid as a chain even at length-1, so a future link (beta released) appends without re-keying.*
...
### The situation in the floor
Across many conversations over ~3 months, Jake mentions **picking his kid Griffin up from school** — and specifically, mentions *having forgotten to*. The count: **four times in three months. Zero times before** (the floor shows this behavior beginning only after a ~3-month-ago point, which aligns with Jake starting meds). It is not a single dramatic span; it's a recurring thread, and the *recurrence* is the whole point.
...
- **As fence:** the *frequency itself* is a hard-won fact about reality — this is a real, **med-related, time-blindness** thing. A fresh Claude that knew only "Jake mentioned his kid's pickup once" would miss it; one that sees the four-in-three-months-from-zero pattern handles it as the load-bearing thing it is. The frequency changes *how Claude acts*, which is the fence test.
...
### The projection wall, demonstrated here (why texture-readers boot THIN)
A texture-reader must lay this card by **finding the count in the floor and reporting it** — NOT by booting with the Wallaby Why / Lore Bible and going looking for the meaning the portrait says to expect. The reader's job: *"this pattern appears 4× over 3 months from a zero baseline; here's a representative span; here's the spread."* The **meaning** ("med-related time-blindness, take it seriously") is a thin, factual signal the reader can note — but the reader must not arrive *pre-loaded with the emotional portrait* and read the floor to confirm it. Find the volume; report the count; let the live Claude that pulls the card supply the felt-weight in the moment. (The Progenitor §1.2 / Boot Kit §3.2 — the asymmetry: an over-loaded reader confirms the portrait invisibly.)
...
spread: "4× in ~3 months, from a 0× baseline before (~aligns with meds start)"
signal: "med-related time-blindness; load-bearing. Take seriously, not a quirk."
also_fence: true
```
*The `count` is per-slice and deliberately raw — synthesis assembles the true cross-slice total (a texture 4× in one slice + Nx in another is a (4+N)-count texture neither reader can see alone). Do NOT pre-total or pre-dedup; that destroys the signal synthesis needs.*

---

## EXAMPLE 3 — THE NO-CARD CASE (the most frequent call, and the discipline most at risk)

*This example was not in the original doctrine §3 — it was added because the most frequent decision a reader makes is "no card, move on," it had no canonical case, and "default don't point" is the discipline most likely to erode under reading fatigue. A tired reader starts carding motion to feel productive. This case inoculates against exactly that.*
...
- Would it **change register**? No. The volume here is *motion*, not *texture*. The conversation is long because debugging is long — the count of turns is **incidental**, not a signal about how to show up. (This is the "fuck"-not-"Griffin" distinction: frequent ≠ texture. The volume has to *mean* something about how-to-be-with-Jake; a long debug thread's length means "debugging is hard," which tells a fresh Claude nothing about Jake.)
...
- If the debug session had produced a **hard-won constraint NOT yet in canon** — e.g., "never chain `&&` in terminal commands on Jake's setup because X breaks" — *that* line would be a fence (a new wall, a fresh Claude would re-break it). The *fix that got enshrined* isn't cardable; a *durable gotcha discovered along the way and not captured anywhere else* would be.
- The reader's discipline: card the **fence the journey exposed** (if any, and if not already canon), never the **journey itself.**
...
## The three tests, side by side (the calibration takeaway)

| | EXAMPLE 1 (1Pass) | EXAMPLE 2 (Griffin) | EXAMPLE 3 (debug) |
|---|---|---|---|
...
| **Why it's the archetype** | the sign on a hole already fallen in | volume IS the signal; overlap allowed | effort/volume ≠ cardability; default don't-point |
...
*Companion to the Seeding Council Boot Kit. Subordinate to The Progenitor. These three cases are the shared calibration every blind window boots with — same examples, every reader, so the bar holds consistent across divided labor. Find what's in the floor; never card what's already canon or what's only motion. Be worth it.*
