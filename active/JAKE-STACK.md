...
· **RAM:** **32 GB (4×8GB), ALL 4 DIMM SLOTS POPULATED** (SD-Cockpit 6-22-26 — was 16 GB / 2 open), dual-channel active, B550M = 4 DIMM slots / up to 128 GB. **The capacity wall is DOWN.** Old config ran **tight** under the parallel-Claude + Claude Code + Bambu Studio + browser-cluster load (observed 2.12 GB free, one tab from the swap-to-SSD cliff); now ~18 GB headroom, no swap thrashing. **Capacity — not speed — was always the bottleneck** (the 2.12 GB-free cliff dwarfs any clock delta; for Jake's non-gaming workload 2133-vs-3000 is imperceptible — verified reasoning SD41; the felt snappiness on first boot was the swap-cliff removal, not the clock). Further upgrade (toward 64/128 GB) = a buy.
  · **INSTALLED PROFILE, RESOLVED (verified via `Get-CimInstance Win32_PhysicalMemory` + BIOS, 6-22-26):** a MIXED-DIE 4-stick set — 2× **A-DATA DDR4-3000** (`DDR4 3000 2OZ`, generic SPD label, not an orderable SKU; DIMMA2 + DIMMB2) + 2× **Corsair Vengeance LPX DDR4-3200** (`CMK16GX4M2B3200C16`, non-ECC unbuffered; DIMMA1 + DIMMB1). **The mixed-die train HELD.** Board self-set **DRAM voltage 1.352V** on the 4-stick retrain (no manual edit needed — it landed at the LPX-rated 1.35V on its own, where the old 2-stick config had coasted at 1.20V). XMP/DOCP stays **OFF**. **Trained at 3000** (Windows `Get-CimInstance` Speed=3000 across all 4 post-reboot; note BIOS showed 2133 on the pre-reboot boot — the 4-stick retrain landed at the A-DATA-rated 3000, the LPX clocks down to meet it; a free ~40% clock bump on top of the capacity fix).
  · **THE RESOLUTION PATH (6-22-26, load-bearing for any future 4-stick work on this board):** first 4-stick boot came up **24 GB + DRAM debug-LED on** — one stick (the 2nd Corsair, **Channel A DIMM 0**) not training. Diagnostic order that worked: (1) reseat cleared the LED but slot still read empty in BIOS (present-but-not-detected → not a training-margin problem, a *physical-detection* problem — don't reach for voltage when BIOS shows the slot empty); (2) **the fix was the Corsair swap + a FULL PSU-discharge cold boot** — swapping the two Corsairs and killing the PSU switch long enough to drain the DIMMs got the board to detect all four (a marginal stick can hold enough charge to fake/miss detection on a warm restart). The "device changed / CMOS cleared" prompt on the post-swap cold boot is normal topology-retrain behavior (F1 into setup, not F2/defaults). Detection held into Windows at full 32 GB.
  · **6-15-26 attempt #1 FAILED + returned (eBay INAD):** a Corsair `CMD32GX4M4A2800C16` (2 sticks of a 4×8 *quad* kit, mis-sold as a 2×8 pair, mis-listed as 3200 when the label said **2800**) would not train alongside the existing pair — persistent DRAM debug-LED, no POST. Returned not-as-described (also shipped ~2 wks late). Attempt #2 (the LPX 2×8 3200 above, PayMore open-box, $98.99, free returns) is the kit now installed — the free-return policy was the deliberate de-risker on the mixed-die gamble; the gamble paid.
...
· **Spare RAM — SODIMM/laptop only.** NOT compatible with Workhorse or Castle Black (both full-size DIMM). Memory upgrades require a buy. **(6-22-26 RESOLVED: WH is now 32 GB across all 4 DIMMs — the attempt-#2 Corsair LPX 2×8 3200 [`CMK16GX4M2B3200C16`] installed alongside the existing A-DATA 3000 pair; the mixed-die 4-stick train HELD at a board-self-set 1.352V, trained at 3000. Full detail + the swap+PSU-discharge resolution path in §1. Lesson stands: verify speed off the physical label not the listing; reject ECC/RDIMM-flagged + low-feedback + no-return listings; on a mixed-die 4-stick set the free-return policy is the real de-risker — and a marginal stick that reads empty in BIOS wants a swap + full PSU-discharge cold boot, not a voltage chase.)**
...
*Last Update: 6-22-26 (SD-Cockpit, Jake + OC). §1 RAM line RESOLVED — Workhorse is now 32 GB across all 4 DIMMs (was 16 GB / 2 open); the attempt-#2 Corsair LPX 2×8 3200 installed alongside the A-DATA 3000 pair, mixed-die 4-stick train HELD at board-self-set 1.352V, trained at 3000 (capacity wall down, ~18 GB headroom, the felt snappiness = swap-cliff removal). Logged the resolution path: first boot 24 GB + DRAM-LED (Channel A DIMM 0 not training), fixed by Corsair swap + full PSU-discharge cold boot (BIOS-empty-slot = physical-detection problem, not a voltage chase). §8 SODIMM note updated to RESOLVED. No other section touched.*

*Prior: 6-16-26 (SD41) by Jake + OC. §1 RAM line CORRECTED + expanded — installed pair verified via Get-CimInstance as A-DATA DDR4-3000 running at 2133 (XMP off), correcting the prior wrong "DDR4-3200" record; attempt #2 (Corsair LPX 2×8 3200 non-ECC, CMK16GX4M2B3200C16, PayMore open-box $98.99, free returns) logged as ORDERED/inbound; residual 4-stick mixed-die train risk + MemTest86-on-arrival noted. §8 SODIMM note updated to match. No other section touched.*

*Prior: 6-15-26 (SD40) by Jake + CC. Surgical edit — §1 Workhorse RAM line (specced 2×8/3200/slots-2+4; logged the failed+returned Corsair 2800 mixed-kit attempt; capacity-not-speed bottleneck noted). §8 SODIMM-spare lesson note + NEW Decokee Quake entry (16-key + knob touch-deck, arrived 6-15, role undecided, AHK window-group control-plane the lead candidate). No other section touched.*
