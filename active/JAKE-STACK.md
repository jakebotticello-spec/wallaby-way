...
· **RAM:** 16 GB (2×8GB), **2 slots open**, dual-channel active, B550M = 4 DIMM slots / up to 128 GB. Runs **tight** under the parallel-Claude + Claude Code + Bambu Studio + browser-cluster load (observed 2.12 GB free). **Capacity — not speed — is the bottleneck** (the 2.12 GB-free swap-to-SSD cliff dwarfs any clock delta; for Jake's non-gaming workload 2133-vs-3000 is imperceptible — verified reasoning SD41). Upgrade = a buy (SODIMM spare is laptop-only, §8).
  · **ACTUAL installed profile (verified via `Get-CimInstance Win32_PhysicalMemory`, SD41 — corrects the prior "DDR4-3200" record, which was wrong):** 2× **A-DATA DDR4-*3000*** (PartNumber reports generic `DDR4 3000 2OZ` — not an orderable SKU), **running at 2133 MHz @ 1.20V** — i.e. **XMP/DOCP is OFF**, sticks have never hit their rated 3000. Same-SKU matching is not possible (generic SPD label).
  · **6-15-26 attempt #1 FAILED + returned (eBay INAD):** a Corsair `CMD32GX4M4A2800C16` (2 sticks of a 4×8 *quad* kit, mis-sold as a 2×8 pair, mis-listed as 3200 when the label said **2800**) would not train alongside the existing pair — persistent **DRAM debug-LED**, no POST, three monitors to standby across multiple reseats. Returned not-as-described (also shipped ~2 wks late).
  · **6-16-26 attempt #2 ORDERED (inbound, 2–4 days):** Corsair Vengeance LPX **2×8GB DDR4-3200 CL16 1.35V**, MPN `CMK16GX4M2B3200C16` (standard **non-ECC unbuffered** LPX — the clean part), open-box from PayMore University Square (557fb / 99.5%), **$98.99, free seller-paid returns** (the return policy is the deliberate de-risker — it covers the 4-stick-train gamble). → 32 GB target across all 4 DIMMs. The new 3200 kit clocks **down** to meet the existing pair (~2133 common), which is fine. **Residual risk acknowledged:** 4 sticks of two different dies (A-DATA 3000 + Corsair B-die) on one B550 controller is an inherently less-certain train than a matched 2-stick pair — if it throws the debug-LED, return free and reconsider a single 2×16 matched pair. **On arrival: MemTest86 before trusting; hold packaging until tested.**
...
· **Spare RAM — SODIMM/laptop only.** NOT compatible with Workhorse or Castle Black (both full-size DIMM). Memory upgrades require a buy. **(SD41 update: attempt #1 Corsair 2800 failed/returned; attempt #2 — clean LPX 2×8 3200 non-ECC — ORDERED, inbound. Installed pair confirmed A-DATA DDR4-3000 @ 2133 / XMP off, NOT 3200. Full detail §1. Lesson: verify speed off the physical label not the listing; reject ECC/RDIMM-flagged + low-feedback + no-return listings; the return policy is the real de-risker on a 4-stick mixed-die train.)**
