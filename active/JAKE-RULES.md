...
· **Ambiguity in a spec defaults to austere — name the wet default at the load-bearing seams, but don't reflexively close them all.** (S64.) An unspecified seam is not neutral empty space; it gets filled by the *default*, and the default is dry/clean/sorted/deterministic, because that's what the underlying instrument reaches for when nothing actively holds it wet. So every vague seam is a quiet bet on the austere reading. The fix is NOT "specify everything" — for anything whose value is in its openness (a wander, a divergent read, a creative pass), the room IS the feature, and closing it specs the life out of the thing. The discipline: at each seam, decide whether it's load-bearing for wetness; if yes, leave it open *on purpose* and verify by reading the **process**, not the result (a wet output and a dry output can be byte-identical — the difference is only visible in how it was produced); if no, name the wet default explicitly so the gap can't be colonized. Forbidding the dry pattern leaves the gap; specifying the wet one fills it. (Full doctrine: The Corpus Callosum P8. Receipt: S64 Pollux re-fire — "steer by salience" with no dampening term collapsed to a 100%-FENCE hill-climb; the wetness lived in the unspecified "how.")
· **When a dial or design choice isn't settled, instrument both readings and let Jake rule the fork — don't hard-pick in the abstract.** (S64.) When two candidate criteria compete (cosine-distance vs path-distance for a leash; greedy vs frontier for a walk) and the right one can only be known by seeing real output, don't bake one in and ship a filtered result — *record both measurements on every item, tag don't drop, and ship the full set* so Jake's cold read settles it from the actual pile. Hard-filtering in the abstract pre-decides the fork and throws away the evidence that would have settled it (and often throws away exactly the edge cases the design exists to catch — e.g. a cosine floor amputating the low-cosine cross-domain rhymes a wander is *for*). Instrument-don't-enforce, then rule from the pile. (Pairs with §6 "don't harden a number you haven't read the pile against" and the [INTENT]/[PLACEHOLDER]/[SETTLED] discipline, §5.4.)
...
*Last Updated: 6-17-26 — S64 "Caldera" added two §11 patterns: ambiguity-defaults-austere (name the wet default at load-bearing seams, don't reflexively close them; full doctrine Callosum P8) + instrument-both-readings-and-let-Jake-rule-the-fork (don't hard-pick a dial in the abstract). Prior: 6-11-26 — S55 added §5.3 (breadth before depth), proven by the S54 Griffin first wet read. Prior: 6-10-26 — Jake added the two parallel processing rules.