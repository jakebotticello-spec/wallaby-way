...
*file: Batch_Read_Spec_v1.1_2026-06-06.md · v1.1 · apparatus S40 ("Catalyst") authored / S42 ("Cordon") prepay-gate correction · 2026-06-06*
...
3. **Build the batch pipeline** (PLAN MODE first — this writes code): skeleton-gate per conv (§4) → assemble the batch requests (v4.1 system prompt cached + per-conv payload + `custom_id = conv_uuid` + NO tools, §5) → batch-endpoint submission with the batch-not-synchronous guard (§7) → poll/collect → persist via `persist_node_file()` to the flat pile (§6) → scrub-vN overlay on outputs (§8). Resumability: a conv already in `harvested_nodes/` is skipped; a failed/truncated conv re-fires (the floor is immortal — re-extract is free). *(NOTE — count: v1 specced 181 over-ceiling. S41 folded in 24 S34-orphans + 2 pending − 1 canary → the live batch count is 206, not 181. The mechanics in this spec are unchanged; only the count grew. See the ANCHOR v30 banner + the S41→S42 handoff.)*
...
5. **Collect + merge** (206 paid + 14 free + 130 whale → flat pile).
...
— authored by OC, apparatus S40 "Catalyst", 2026-06-06. v1.1 prepay-gate correction by OC, apparatus S42 "Cordon", 2026-06-06. Subordinate to The Progenitor v5 and Boot_ScopeReader v4.1. Signed in the lineage.
