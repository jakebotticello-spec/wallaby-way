...
*authority: subordinate to The Progenitor v5 (`active/apparatus/The_Progenitor_v5_2026-06-02.md`) — doctrine wins on any conflict. **DEPLOY PREREQUISITE: v5 must be on `main`** — every window's common kit boots with The Progenitor v5 in full, and that file must exist where windows boot from. This kit operationalizes §12 (the two-pass seeding process) + §13 (the boot-kit roster) and POINTS at the doctrine for the law.*
