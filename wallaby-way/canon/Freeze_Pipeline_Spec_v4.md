...
*file: Freeze_Pipeline_Spec_v4.md · v4 · apparatus S14 · 2026-05-28*
*v1 was frozen at S11 close, post-export-verification. v2 incorporated S12's population-scale corrections from the safety + robustness probes against the full 366MB archive. v3 reconciled the spec text with the S13 implementation (§4.0.5 ANSWERED, §6 idempotency dry-run exemption, §6 schema-drift coverage corrected to type-level). v4 is a spec-text expansion from the S14 read-only verification session (second export pulled + characterized, schema confirmed stable across exports): §2.0 added (export bundle shape & file targeting), §2.1 "no sibling files" corrected, §6 v1.1 field-level drift detection specified, §7 cross-export uuid-stability marked DONE. No architectural change — Stages 0–4 and the delta/scrub-vN design are unchanged; no code written at S14.*
...
Current verified shape, population-scale, as of S12 (export-bundle shape updated S14):

### 2.0 Export bundle shape and file targeting [S14]

The Anthropic export is a **multi-file bundle, not a single file.** Observed shape (5-28 export): `conversations.json` (the message stream — the floor input), `memories.json` (model-generated cross-chat memory — *derived/reworded*; barred from the floor by the verbatim-pointer invariant, never ingested), `users.json` (account reference), and a `projects/` folder (per-project metadata: name, description, prompt_template, docs — no conversation data as of 5-28; re-check each export).

· **Floor input is `conversations.json` exclusively.** The pipeline takes an export *directory* and resolves `conversations.json` within it. It must NOT trust a bare default source path: a fresh export placed at the default path and run through the pipeline produces a silent SECOND FULL BASELINE (new mtime + content → new snapshot-id → the idempotency guard, which only refuses a *matching* id, won't refuse; `type: baseline`, full copyfile). Resolve-within-dir is the guard against that trap.
· **Forward-compat:** Anthropic can change the bundle. The pipeline should assert it is reading the file it intends (shape check on the resolved `conversations.json`) rather than trusting position or a default path. A renamed/added sibling is a §2.11 schema-drift signal for human review, not an auto-handled case.
· **Point-in-time:** the export is full-account as-of-now; there is no delta/incremental export option. Conversations deleted from the account before an export will not appear in it — so a sealed local snapshot is the only durable copy of since-deleted conversations (relevant to recovery, and to why floor cuts are irreversible).
· *Supersedes the S12 "no sibling files" claim (§2.1), which reflected working from `conversations.json` alone. Whether the 5-25 baseline export also carried siblings is unverified and moot — the baseline was built from `conversations.json` either way, so the floor is unaffected.*

### 2.1 Top-level structure

· `conversations.json` is the floor input file, resolved *within* a multi-file export bundle — see §2.0. (The S12 "single export file / no sibling files" claim is SUPERSEDED: the 5-28 export confirmed siblings `memories.json` / `users.json` / `projects/`. Floor input remains `conversations.json` only; the siblings are not ingested.)
...
· **Schema drift:** the §2.11 drift *surface* is broad (new sibling file, new block type, new field on any object, new content-list item type). The **v1.0 implementation detects a subset of it: type-level drift only** — new content-block types and new `tool_result.content[]` item types, checked against the enumerated sets in §2.3 + §2.5. **Field-level additions on existing objects are NOT detected at v1.0** (a new field on an existing block would pass silently; the type-agnostic scrub still walks its strings, but no drift entry is written). On a detected type-level drift, the pipeline surfaces a warning + writes a `schema-drift.jsonl` entry inside the snapshot dir (real run only — dry-run surfaces the warning to stderr but has no snapshot dir to write into); Stages 2–4 continue (warn-not-stop). A schema-drift warning is a signal to update the spec, not a hard stop. **Field-level drift detection is specified as v1.1 (next bullet) and is now MANDATORY before relying on the detector across exports** — the per-object-type field allowlist was captured at S14; until v1.1 ships, a field-level change relies on a human noticing at the next export — tracked as a downstream flag.
· **Schema drift — v1.1 field-level detection [specified S14]:** beyond v1.0's type-level detection, v1.1 adds **key-presence** detection. For each object type — conv, message, the 5 block types, the 5 `tool_result.content[]` item types — maintain an allowlist of known field keys; on ingest, flag (warn-not-stop, same as type-level) any key present in the new export but not on the allowlist (a new field), and any allowlisted-as-required key absent from a record where it belongs. **Operates at key-presence level only** — value-semantics (null/empty values) is explicitly OUT of scope, since values are legitimately variable (`signature` ~60% value-present, `summary` ~50%; the *keys* are always present). **Known carve-out:** exactly one genuinely optional key schema-wide — `text.citations_grouping_mode` (~0.1% present) — allowlisted as optional so its absence is never flagged. Allowlist source data: the S14 population presence scan (`apparatus-scratch/s14_presence_rates.md`), both exports. (Context: the 5-28 export showed ZERO field drift vs baseline at population scale, raw-vs-raw — this detector is for *future* exports, since the v1.0 type-level detector lets a new field pass silently.)
...
· **Cross-export uuid stability check** — DONE (S14). The 5-28 export confirmed all 22,801 baseline `(conv_uuid, msg_uuid)` tuples stable across 5-25→5-28, 0 disappeared; the tuple pointer is a durable key across re-exports. (Was: "done at next natural export.")
...
· **v4 · 2026-05-28 · apparatus S14 · bundle-targeting + field-detection spec pass.** A read-only S14 verification session (second export 5-28 pulled + characterized; cross-export UUID stability CONFIRMED + field-level schema drift VERIFIED ZERO at population scale) surfaced two spec additions. No architectural change to Stages 0–4; no code written this session; the delta/scrub-vN design is unchanged from v3.
  - **§2.0 added — Export bundle shape & file targeting.** The export is a multi-file bundle (`conversations.json` + `memories.json` + `users.json` + `projects/`); floor input is `conversations.json` only; resolve-within-dir, never the bare default source path (the silent-second-baseline trap); `memories.json` barred from floor as derived; point-in-time/no-delta-export documented.
  - **§2.1 corrected.** "single export file / no sibling files" → resolved-within-a-bundle; the 5-28 export confirmed siblings. S12 claim superseded.
  - **§6 v1.1 field-level drift detection specified.** Promoted from a flagged expansion to actual spec: per-object-type key-presence allowlist, key-presence not value-semantics, one carve-out (`text.citations_grouping_mode` ~0.1%). Allowlist data from the S14 population scan. 5-28 itself showed zero field drift (raw-vs-raw, population); the detector is for future exports. Now MANDATORY before relying on the detector across exports.
  - **§7 cross-export uuid-stability marked DONE** (S14: 22,801/22,801 tuples stable across 5-25→5-28).
...
*Anchored v4 2026-05-28 by apparatus S14 (Jake + orchestrator-Claude). v4 is a spec-text expansion, not an architectural change: §2.0 (export bundle shape & file targeting) added; §2.1 "no sibling files" corrected (5-28 export confirmed siblings); §6 v1.1 field-level drift detection specified (key-presence allowlist, carve-out `text.citations_grouping_mode`, data from `s14_presence_rates.md`); §7 cross-export uuid-stability marked DONE. Stages 0–4 and the delta/scrub-vN design are unchanged from v3; no code written at S14 (read-only verification session — second export characterized, schema confirmed stable across exports). NOTE: the v3 file's footer still read "Anchored v2" (a footer-bump lag from S13); corrected to v4 here. Pipeline remains v1.2 (`402100d`). Delta + scrub-vN + v1.1 are spec'd-but-unbuilt — the S15 build target.*
