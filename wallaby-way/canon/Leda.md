...
`chunk_embeddings.npy`, the kNN edges (`edges.json`, **38,171** cross-conv synapses), the UMAP layout — all real, all stored, all queryable. **None of it is any picker's substrate.** Those edges are **cosine ≥ 0.30 — convergent, high-similarity, semantically-NEAR pairs** (S57/N8). The catch a picker hunts is the **cross-domain rhyme — LOW cosine by definition, same shape, different surface.** Walking the geometry routes a picker to topic-relatedness and rebuilds a convergent read. The geometry shows what is *near*; the pickers hunt what *rhymes*. **The embedding web is forbidden to every picker** — not because it is unavailable, but because using it quietly converts a divergent arm into a slow convergent one (Bouquet §2: salience over relevance; the instant it follows similarity it has a target).
